...
document.addEventListener("DOMContentLoaded", function (event) {
	...
	const repairTypesSection = document.querySelectorAll('.repair-types');

	if(repairTypesSection.length) {
		let repairElements = document.querySelectorAll(".repair-types__content");

		for (let index = 0; index < repairElements.length; index++) {
			new SimpleBar( repairElements[index], {
				autoHide: false,
			});
		}
